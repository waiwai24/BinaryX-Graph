...
pub async fn handle_query(query_type: QueryType, config: Config) -> Result<()> {
    ...
    match query_type {
        QueryType::Functions { pattern, binary, limit, format } => {
            query_functions(&session, &pattern, binary.as_deref(), limit, &format).await?
        }
        QueryType::Binary { binary_name, format } => {
            query_binary(&session, &binary_name, &format).await?
        }
        QueryType::Callgraph { function_name, binary, show_callees, show_callers, max_depth, format } => {
            query_callgraph(&session, &function_name, binary.as_deref(), show_callees, show_callers, max_depth, &format).await?
        }
        QueryType::Xrefs { address, binary, format } => {
            query_xrefs(&session, &address, binary.as_deref(), &format).await?
        }
        QueryType::CallPath { function_name, binary, show_paths, show_sequences, show_recursive, show_upward, show_context, max_depth, format } => {
            query_call_paths(&session, &function_name, binary.as_deref(), show_paths, show_sequences, show_recursive, show_upward, show_context, max_depth, &format).await?
        }
    }
...
async fn query_functions(
    ...
    if let Some(binary_name) = binary {
        println!("Querying functions with pattern: '{}' in binary: '{}'", pattern, binary_name);
    } else {
    ...
    if format == "json" {
        ...
        println!("{:<40} {:<20} {:<15} {:<20} {:<15}", "Name", "Type", "Address", "Binary", "UID");
        println!("{}", "-".repeat(110));

        for f in &functions {
            let binary_display = extract_binary_from_uid(&f.uid);
            println!("{:<40} {:<20} {:<15} {:<20} {:<15}",
                f.name,
                format!("{:?}", f.r#type),
...
async fn query_callgraph(
    ...
    if let Some(binary_name) = binary {
        println!("Querying call graph for function: '{}' in binary: '{}' (depth: {})", function_name, binary_name, max_depth);
    } else {
        println!("Querying call graph for function: '{}' (depth: {})", function_name, max_depth);
    }

    let callgraph = session.query_callgraph_with_depth(function_name, binary, max_depth).await?;
    ...
    if format == "json" {
        ...
        if callgraph.callees.is_empty() && callgraph.callers.is_empty() {
            println!("No call graph information found for function: '{}'", function_name);
        } else {
            println!("\nSummary: {} callees, {} callers", callgraph.callees.len(), callgraph.callers.len());
        }
    }
...
async fn query_xrefs(
    ...
    if let Some(binary_name) = binary {
        println!("Querying cross-references for address: '{}' in binary: '{}'", address, binary_name);
    } else {
    ...
    if format == "json" {
        ...
        println!("{:<30} {:<30} {:<15}", "From Function", "To Function", "Offset");
        println!("{}", "-".repeat(75));
...
async fn query_call_paths(
    ...
    if let Some(binary_name) = binary {
        println!("Analyzing call paths and execution order for function: '{}' in binary: '{}'", function_name, binary_name);
    } else {
        println!("Analyzing call paths and execution order for function: '{}'", function_name);
    }

    let analyzer = crate::neo4j::CallPathAnalyzer::new(session.importer().connection().clone());

    let show_all = !show_paths && !show_sequences && !show_recursive && !show_upward && !show_context;

    if show_paths || show_all {
        println!("\nAnalyzing call paths...");
        let call_paths = analyzer.query_call_paths(function_name, max_depth).await?;
        ...
        if call_paths.is_empty() {
            ...
            if call_paths.len() > 10 {
                println!("  ... and {} more paths (use --format json to see full list)", call_paths.len() - 10);
            }
        }
    }

    if show_sequences || show_all {
            ...
            for recursive in &recursive_calls {
                match recursive.call_type {
                    RecursiveCallType::Direct => {
                        println!("  Direct recursion: {} (Depth: {})",
                            recursive.function_name,
                            recursive.depth
                        );
                    }
                    RecursiveCallType::Indirect => {
                        println!("  Indirect recursion: {} (Depth: {})",
                            recursive.function_name,
                            recursive.depth
                        );
                    }
    ...
    if show_upward || show_all {
        println!("\nAnalyzing upward call chains...");
        let upward_chains = analyzer.query_upward_call_chain(function_name, max_depth).await?;
        ...
        if upward_chains.is_empty() {
            ...
            for (i, chain) in sorted_chains.iter().take(10).enumerate() {
                if let Some(target) = chain.target_function() {
                    println!("  Call chain {}: {} (Depth: {})", i + 1, target.name, chain.length);
                    ...
                    if format == "json" {
                        ...
                        for node in &chain.nodes {
                            ...
                            let arrow = if node.depth < chain.length - 1 { "├─" } else { "└─" };
                            println!("{}{} {} @ {}", 
                                indent, 
                                arrow,
                                node.name, 
            ...
            if upward_chains.len() > 10 {
                println!("  ... and {} more call chains (use --format json to see full list)", upward_chains.len() - 10);
            }
    ...
    if show_context || show_all {
        println!("\nFull call context analysis...");
        let context_analysis = analyzer.analyze_call_context(function_name, max_depth).await?;
    ...
    if format == "json" {
        let enhanced_graph = analyzer.query_enhanced_call_graph(function_name, max_depth).await?;
